...
static const uint32_t TableCRC32[CRC32_TABLE_SIZE] =
{
...
GUID GUID_EMPTY      = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
GUID GUID_LEGACY_MBR = { 0x41, 0xDE, 0x4D, 0x02, 0xE7, 0x33, 0xD3, 0x11, 0x9D, 0x69, 0x00, 0x08, 0xC7, 0x81, 0xF3, 0x9F };
GUID GUID_EFI        = { 0x28, 0x73, 0x2A, 0xC1, 0x1F, 0xF8, 0xD2, 0x11, 0xBA, 0x4B, 0x00, 0xA0, 0xC9, 0x3E, 0xC9, 0x3B };
GUID GUID_MYTHE      = { 0x57, 0x00, 0xE9, 0xE6, 0xB5, 0x11, 0x1C, 0x4E, 0xB5, 0x78, 0x10, 0x1F, 0x7D, 0x2F, 0x4E, 0xFD };
...
void ItGUID(GUID guid)
{
    ...
    guid[6] = (guid[6] & 0x0F) | 0x40; // UUID Version
    guid[8] = (guid[8] & 0x3F) | 0x80; // UUID Variant
}

CRC32 ItCRC32(const void* pData, size_t szData)
{
    const uint8_t* pDataBytes = (const uint8_t*) pData;
    CRC32 CRC = 0xFFFFFFFF;

    for (size_t i = 0; i < szData; i++)
    {
        uint8_t index = (uint8_t)((CRC ^ pDataBytes[i]) & 0xFF);
...
UnpackCHS ItUnpackCHS(CHS chs)
{
    UnpackCHS result;
    result.Head     = chs[0];
    result.Sector   = chs[1] & 0x3f;
    result.Cylinder = ((chs[1] & 0xC0) << 2) | chs[2];
    return result;
}
...
void ItLBAToCHS(LBA32 LBA, CHS chs)
{
    ...
    const uint16_t HEADS_PER_CYLINDER = 16;

    uint16_t cylinder = LBA / (HEADS_PER_CYLINDER * SECTORS_PER_TRACK);
    LBA = LBA % (HEADS_PER_CYLINDER * SECTORS_PER_TRACK);
    uint8_t head = LBA / SECTORS_PER_TRACK;
    uint8_t sector = (LBA % SECTORS_PER_TRACK) + 1;

    if (cylinder > 1023 || head > 255 || sector > 63)
    {
        chs[0] = 0xFF;
        chs[1] = 0xFF;
        chs[2] = 0xFF;
...
void ItWriteAnsiToPartName(uint16_t pDest[GPT_PARTITION_NAME_LOGICAL_SIZE], const char* pSrc)
{
    ...
    size_t i = 0;
    for (; i < GPT_PARTITION_NAME_LOGICAL_SIZE && *pSrc != '\0'; i++)
    {
        *pDestBytes++ = *pSrc++;
        *pDestBytes++ = 0x00;
    }
    for (; i < GPT_PARTITION_NAME_RAW_SIZE; i++)
    {
        *pDestBytes++ = 0x00;
    }
}

void ItPrintMbrPartition(MBR_PARTITION_ENTRY* pEntry)
{
    UnpackCHS start = ItUnpackCHS(pEntry->StartCHS);
    UnpackCHS end = ItUnpackCHS(pEntry->EndCHS);
    const char* pTypeDesc = pEntry->Type == MBR_PARTITION_TYPE_UEFI_SYSTEM ? "UEFI System Partition" : pEntry->Type == MBR_PARTITION_TYPE_GPT_PROTECTIVE ? "GPT Protective" : "Non-EFI standard value.";
    printf
    (
        "  Attributes: 0x%02X\n"
        "  StartCHS: %u, %u, %u\n"
        ...
        "  StartLBA: %u\n"
        "  Size: %u\n",
        pEntry->Attributes, start.Cylinder, start.Head, start.Sector, pEntry->Type, pTypeDesc,
        end.Cylinder, end.Head, end.Sector,
        pEntry->StartLBA, pEntry->Size
    );
}

void ItPrintMasterBootRecord(MASTER_BOOT_RECORD* pMasterBootRecord, const char* pLeadingStr)
{
    uint16_t pNumBytesOfCode = 0;
    for (uint16_t i = 0; i < MBR_BOOT_CODE_SIZE; i++)
    {
        if (0 != pMasterBootRecord->BootCode[i])
        {
            pNumBytesOfCode++;
            break;
        }
    ...
    if (0 != pNumBytesOfCode)
    {
        printf("%s:\n Boot Code Status: %u Byte(s) of Code\n", pLeadingStr, pNumBytesOfCode);
    }
    else
    {
        printf("%s:\n Boot Code Status: No Boot Code\n", pLeadingStr);
    }
    
    for (int i = 0; i < MBR_NO_PARTITION_ENTRIES; i++)
    {
        MBR_PARTITION_ENTRY* pPartition = pMasterBootRecord->PartitionTable + i;
        if
        (
            pPartition->Attributes  == 0 &&
            pPartition->StartCHS[0] == 0 &&
            pPartition->StartCHS[1] == 0 &&
            pPartition->StartCHS[2] == 0 &&
            pPartition->Type        == 0 &&
            pPartition->EndCHS[0]   == 0 &&
            pPartition->EndCHS[1]   == 0 &&
            pPartition->EndCHS[2]   == 0 &&
            pPartition->StartLBA    == 0 &&
            pPartition->Size        == 0
        )
        {
            printf(" Partition %d: Not Present\n", i+1);
            continue;
        }

        printf(" Partition %d:\n", i+1);
        ItPrintMbrPartition(pPartition);
    }
    printf(" Boot Signature: %s (Value on Disk 0x%02X)\n", pMasterBootRecord->BootSignature == MBR_BOOT_SIGNATURE ? "Present" : "Not Present", pMasterBootRecord->BootSignature);
}
